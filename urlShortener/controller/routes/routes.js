...
router.get("/new/:url(*)", async (req, res, next) => {
	...
	const isValid = await urlAPI.validateUrl(url);
	debug("The url format is: ", isValid);

	if (isValid) {
		// call the api to create short url
		const shortUrl = await urlAPI.addShortUrl(url);
...
router.get("/:shortened", async (req, res, next) => {
	const shortLink = req.params.shortened;
	...
	const urlExistence = await urlAPI.getLongURl(shortLink);
